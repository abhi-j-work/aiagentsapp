...
const AnimatedBackground = () => {
  ...
  useEffect(() => {
    ...
    const updateWaveData = () => { waveData.forEach(data => { if (Math.random() < 0.01) { data.targetValue = Math.random() * 0.7 + 0.1; } const diff = data.targetValue - data.value; data.value += diff * data.speed; }); };
    ...
    const draw = () => {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const r = 79 + intensity * 100; const g = 70 + intensity * 130; const b = 229;
