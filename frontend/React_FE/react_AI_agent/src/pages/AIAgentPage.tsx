...
// This type should match the EvaluationResult in your models.py/api.ts
        ...
        <div className="space-y-4 max-h-48 overflow-y-auto pr-2 bg-slate-800/60 rounded-md p-3">
            {Object.entries(schema.tables).map(([tableName, tableData]) => (
                <div key={tableName} className="text-sm">
                    ...
                    <ul className="mt-2 pl-4 space-y-1.5">{tableData.columns.map(col => <li key={col.column_name} className="text-slate-300 flex items-center"><ChevronsRight className="w-3 h-3 mr-2 text-indigo-500" />{col.column_name} <span className="ml-2 text-indigo-400/70">({col.data_type})</span></li>)}</ul>
                </div>
            ))}
        </div>
    </div>
...
const EvaluationSummary = ({ evaluation }: { evaluation: EvaluationResult }) => {
    ...
    return (
        <div className="animate-fade-in p-4 bg-slate-800/50 border border-slate-700 rounded-lg">
            <h4 className="font-semibold text-white flex items-center gap-2 mb-2 text-base">
                ...
                AI Judge: Classification Quality
            </h4>
            <div className="flex items-start gap-4">
                <div className={`flex items-center gap-1 font-bold text-lg ${getStarColor(evaluation.score)}`}>{evaluation.score} / 10 <Star className="w-5 h-5 fill-current" /></div>
...
const SqlDisplay = ({ statements }: { statements: string[] }) => (
    <div className="animate-fade-in"><h4 className="font-semibold text-white flex items-center gap-2 mb-3"><FileText className="w-4 h-4 text-indigo-400" />AI Generated SQL Plan</h4><div className="max-h-64 overflow-y-auto bg-black p-4 rounded-md border border-slate-700"><pre className="text-sm text-slate-200 whitespace-pre-wrap"><code>{statements.join('\n\n---\n\n')}</code></pre></div></div>
);
...
const GovernedViewList = ({ views, onSelectView, isLoading, selectedView }: { views: string[], onSelectView: (viewName: string) => void, isLoading: boolean, selectedView: string | null }) => (
    <div className="animate-fade-in"><h4 className="font-semibold text-white flex items-center gap-2 mb-3"><Eye className="w-4 h-4 text-indigo-400" />Governed Views</h4><div className="space-y-2 bg-slate-800/60 rounded-md p-3">{views.map(viewName => (<button key={viewName} onClick={() => onSelectView(viewName)} disabled={isLoading} className={`w-full text-left text-sm p-2 rounded-md transition-colors flex items-center gap-2 disabled:opacity-50 ${selectedView === viewName ? 'bg-indigo-600 text-white' : 'bg-slate-900/70 text-slate-300 hover:bg-slate-700'}`}>{isLoading && selectedView === viewName ? <LoaderCircle className="w-4 h-4 animate-spin" /> : <Table className="w-4 h-4 text-slate-400" />} {viewName}</button>))}</div></div>
);

const ViewDataDisplay = ({ viewData }: { viewData: FetchViewDataResponse }) => {
    if (!viewData.data || viewData.data.length === 0) return <div className="text-center text-slate-300 p-4 mt-4 animate-fade-in">No data to display.</div>;
...
const AIAgentPage = () => {
    ...
    const [classificationEvaluation, setClassificationEvaluation] = useState<EvaluationResult | null>(null);
    ...
    const handleRunAnalysis = async () => {
        ...
        setDownloadableIntegrityData(null); setDownloadableSqlData(null);
        try {
    ...
    const handleClassifyData = async () => {
        ...
        try {
            const res: any = await postClassifyData(schema);
            setClassifications(res.classification_results);
            if (res.evaluation) { setClassificationEvaluation(res.evaluation); }
    ...
    const handleListGovernedViews = async () => {
        ...
        try { const res = await postListGovernedViews(connectionString); setGovernedViews(res.governed_views); }
    ...
    const handleFetchViewData = async (viewName: string) => {
        ...
        try { const res = await postFetchViewData(connectionString, viewName, viewingRole); setViewData(res); }
