    ...
    useEffect(() => {
        ...
        const runAllSteps = async () => {
            try {
                ...
                const hasFailedChecks = qualityResults.validation_results.some(result => !result.is_valid);
                ...
                if (hasFailedChecks) {
                    ...
                    // ======================================================================
                    // ===== THE FIX: Pass the entire `qualityResults` object directly ======
                    // ======================================================================
                    // The backend expects the full report object, not just the failed checks.
                    // This resolves the 422 Unprocessable Entity error.
                    const remediationPlan: GenerateRemediationResponse = await postGenerateRemediationSql(
                        connectionString,
                        qualityResults // Pass the full report object
                    );
                    // ======================================================================

                    if (!remediationPlan.remediation_plan || remediationPlan.remediation_plan.length === 0) {
                        console.warn("Quality checks failed, but no remediation plan was generated by the AI.");
                        setCurrentStep('Step 5/5: No applicable remediation actions were generated.');
                    } else {
                ...
                if (viewsResponse.filtered_views.length > 0) {
                    setSelectedView(viewsResponse.filtered_views[0]);
                }
        ...
        <div className="min-h-[calc(100vh-80px)] w-full flex items-center justify-center p-8">
            <div className="w-full max-w-4xl mx-auto">
                <div className="card-border rounded-2xl p-8">
                    <h2 className="text-2xl font-semibold text-white text-center mb-6">Automated Data Quality Run for: <span className="text-indigo-400 font-bold">{tableName}</span></h2>
                    ...
                    {isComplete && (
                        <div className="text-center animate-fade-in">
                            ...
                            <p className="text-slate-300 mt-2 mb-8">The automated data quality process has finished. Review the summary and the created views.</p>
                            ...
                            <div className="pt-8 mt-8 border-t border-slate-700/80 flex justify-center">
                                <Link to="/data-quality" className="group btn-secondary flex items-center text-sm font-semibold px-4 py-2 rounded-lg bg-indigo-900/50 text-indigo-300 hover:bg-indigo-900 border border-indigo-700/50">
                                    <RefreshCw className="w-4 h-4 mr-2 transition-transform duration-300 group-hover:rotate-180"/> Start New Analysis
                                </Link>
                            </div>
                        </div>
                    )}
