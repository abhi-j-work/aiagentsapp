...
// A small, reusable component for the floating table cards inside the visualizer
const TableCard = ({ title, color, animationClass }: { title: string, color: string, animationClass: string }) => (
  <div className={`w-16 h-12 glass rounded-lg gradient-border shadow-lg overflow-hidden ${animationClass}`}>
    ...
    <div className="px-1.5 py-0.5 space-y-0.5">
      <div className="flex items-center space-x-0.5">
        <KeyRound className="text-yellow-400 h-2 w-2" />
...
const DataGovernancePage = () => {
  return (
    // Main container to center the card on the full-screen black background
    <div className="fixed inset-0 flex items-center justify-center p-4 z-10">
      <div className="w-full relative max-w-xs">
        {/* The main card with its border and floating animation */}
        <div className="relative card-border overflow-hidden rounded-2xl flex flex-col animate-float">
          
          {/* Top Section: The Database Schema Visualizer */}
          <div className="p-4 flex justify-center relative">
            <div className="w-full h-48 rounded-xl gradient-border inner-glow overflow-hidden relative">
              ...
              {/* SVG for database connection lines */}
              ...
              {/* Animated Floating Database Tables */}
              <div className="absolute inset-0 w-full h-full">
                ...
                <div className="absolute left-3 top-12">
                  <TableCard title="users" color="from-indigo-500/20 to-blue-500/20" animationClass="animate-table-float-1" />
                </div>
                <div className="absolute right-3 top-12">
                  <TableCard title="orders" color="from-blue-500/20 to-purple-500/20" animationClass="animate-table-float-2" />
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 top-24">
                  <TableCard title="products" color="from-purple-500/20 to-pink-500/20" animationClass="animate-table-float-3" />
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 bottom-3">
                   <TableCard title="analytics" color="from-orange-500/20 to-red-500/20" animationClass="animate-table-float-4" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4">
            ...
            <p className="text-white/70 mb-4 leading-relaxed text-xs font-inter">
